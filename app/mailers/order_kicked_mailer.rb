class OrderKickedMailer < ApplicationMailer
  default from: "KickParty <#{ENV["EMAIL_ACCOUNT"]}>"

  def order_kicked_email(user, order, event, payment_type, number)
    ...
    mail(to: "#{@user.first_name} #{@user.last_name} <#{@user.email}>", subject: "#{@event.name} has Kicked!")
  end
end
